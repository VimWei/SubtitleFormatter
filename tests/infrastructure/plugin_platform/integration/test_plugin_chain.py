...
class TestPluginChain:
    ...
    def setup_method(self):
        ...
        with patch("plugins.builtin.punctuation_adder.plugin.PunctuationModel") as mock_model:
            ...
            self.punctuation_adder._model.restore_punctuation.return_value = (
    ...
    def test_plugin_chain_with_custom_configs(self):
        ...
        text_cleaning = TextCleaningPlugin(
            {"normalize_punctuation": False, "normalize_numbers": False}
        )

        punctuation_adder = PunctuationAdderPlugin(
            {"capitalize_sentences": False, "split_sentences": False}
        )

        text_to_sentences = TextToSentencesPlugin({"filter_ellipsis_only": True})

        sentence_splitter = SentenceSplitterPlugin({"max_degradation_round": 3})
