    ...
    def _validate_config(self) -> None:
        ...
        for field, validator in field_validators.items():
            if field in self.config:
                try:
                    ...
                    if result is False:
                        errors.append(
                            f"Field '{field}' validation failed: value '{self.config[field]}' is invalid"
                        )
        ...
        if errors:
            raise PluginConfigurationError(
                ...
                invalid_fields=[
                    field for field in self.config.keys() if field not in allowed_fields
                ],
            )
...
class PluginProcessingError(PluginError):
    ...
    def __init__(
        self, message: str, plugin_name: str = None, input_data: str = None, cause: Exception = None
    ):
