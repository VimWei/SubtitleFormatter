...
class SentenceHandler:
    ...
    def _fix_word_spacing(self, tokens):
        ...
        for i, token in enumerate(tokens):
            ...
            if i > 0:
                ...
                # 检查是否需要空格
                if (
                    current_text in no_space_before
                    or prev_text in no_space_after
    ...
    def _split_by_grammar(self, sent):
        ...
        for token in sent:
            ...
            if current_tokens and (
                self._is_break_point(token) or self._should_start_new_sentence(token)
            ):
    ...
    def _should_start_new_sentence(self, token):
        ...
        # 1. 句子开头的连词
        if (
            token.text.lower() in {"and", "but", "or", "so", "because", "if", "though", "as"}
            and token.i > 0
        ):
            # 检查是否完整的从句结构
            has_subject = False
