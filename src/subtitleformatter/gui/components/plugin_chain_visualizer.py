...
class PluginChainVisualizer(QWidget):
    ...
    def update_plugin_chain(self, plugin_chain: List[str], plugin_metadata: Optional[Dict[str, Dict]] = None):
        """更新插件链显示"""
...
class PluginChainCanvas(QWidget):
    ...
    def _calculate_nodes_per_row(self, node_widths: List[int], available_width: int, spacing: int, arrow_width: int) -> int:
        """计算每行能容纳的节点数"""
        if not node_widths:
            return 1
    ...
    def draw_plugin_chain(self, painter: QPainter):
        ...
        nodes_per_row = self._calculate_nodes_per_row(node_widths, available_width, spacing, arrow_width)
        ...
        for i, plugin_name in enumerate(self.plugin_chain):
            ...
            if col < len(row_widths) - 1:
                ...
                self.draw_arrow(painter, arrow_x, y + node_height // 2, 
                               arrow_x + arrow_width, y + node_height // 2)
            
            # 绘制垂直箭头（除了最后一行，且是每行最后一个节点）
            if row < num_rows - 1 and col == len(row_widths) - 1:
                arrow_y = y + node_height + spacing // 2
                self.draw_arrow(painter, x + node_widths[i] // 2, arrow_y, 
                               x + node_widths[i] // 2, arrow_y + spacing, vertical=True)
            
            # 绘制插件节点
            self.draw_plugin_node(painter, x, y, node_widths[i], node_height, plugin_name)
    
    def draw_plugin_node(self, painter: QPainter, x: int, y: int, width: int, height: int, plugin_name: str):
        """绘制插件节点"""
    ...
    def draw_arrow(self, painter: QPainter, start_x: int, start_y: int, end_x: int, end_y: int, vertical: bool = False):
        """绘制箭头"""
