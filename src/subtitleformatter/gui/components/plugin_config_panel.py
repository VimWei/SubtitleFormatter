...
class PluginConfigPanel(QWidget):
    ...
    def create_control_by_type(self, param_type: str, param_config: Dict[str, Any], default_value: Any, param_name: str) -> Optional[QWidget]:
        """根据类型创建控件"""
        if param_type == "boolean":
            control = QCheckBox()
            ...
            control = QSpinBox()
            control.setRange(param_config.get("minimum", -999999), param_config.get("maximum", 999999))
            control.setValue(int(default_value) if default_value != "" else 0)
            return control
        
        elif param_type == "number":
            control = QDoubleSpinBox()
            control.setRange(param_config.get("minimum", -999999.0), param_config.get("maximum", 999999.0))
    ...
    def export_configuration(self):
        ...
        try:
            ...
            # 这里可以添加文件保存对话框
            logger.info(f"Exported configuration for plugin '{self.current_plugin}':\n{config_json}")
        except Exception as e:
