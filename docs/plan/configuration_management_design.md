...
- **配置加载逻辑**: 当插件配置文件不存在时，从 `plugin.json` 提取默认配置，而不是使用空配置

### 11.2 插件链配置的完整保存策略
**关键结论**: 插件链配置 - 全量保存所有用到的插件的完整配置。
...
- **配置完整性**: 插件链配置中包含所有插件的配置节，即使某些插件没有配置参数（返回空配置 `{}`）
- **无配置插件处理**: 如果插件没有 `config_schema` 或没有配置参数，会返回空配置 `{}`，这在插件链配置中仍然会保留配置节
...
- **默认配置来源**: 从 `plugin.json` 的 `config_schema` 获取，不是从 `default_plugin_chain.toml`
- **全量保存策略**: 插件链配置包含所有插件的完整配置，便于用户查看和修改
