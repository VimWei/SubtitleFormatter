...
- **智能保存**: 根据插件选择来源决定保存位置
  ...
  - 来自插件链: 保存到插件链工作配置（实时更新）
- **存储策略**: 插件自定义配置立即保存，插件链配置延迟保存
- **配置优先级**: 插件链工作配置 > 插件链保存配置 > 插件自定义配置 > 内置默认配置
...
2. **智能保存机制**: 根据插件选择来源决定保存位置
   ...
   - **来自插件链**: 保存到插件链工作配置（实时更新内存，延迟保存到文件）
3. **配置状态管理**: 工作配置、保存配置、快照配置三层分离
   - 工作配置实时更新，保存配置延迟保存
   - 快照配置在启动时创建，用于 Restore Last 功能保护
4. **配置验证**: 配置变更时进行有效性验证
5. **配置同步**: 确保界面与配置数据同步
...
2. **插件链保存**: 
   - 自动保存未保存的工作配置到插件链文件
   - 如果用户显式保存了插件链，保存到 `data/configs/plugin_chains/` 目录的指定文件
   - 程序退出时，保存到 `chain_latest.toml`（不生成新文件）
3. **配置引用更新**: 更新统一配置中的 `current_plugin_chain` 引用
4. **插件配置**: 插件自定义配置已在运行时立即保存到 `data/configs/plugins/` 目录，无需额外处理
5. **配置状态检查**: 检查是否有未保存的工作配置变更
6. **配置备份**: 重要配置变更前创建备份
7. **配置清理**: 清理临时配置和缓存
...
- ✅ **快照保护机制**: Restore Last 功能通过快照机制保护原始配置
- ✅ **自动保存机制**: 实时保存和退出时自动保存未保存的工作配置
