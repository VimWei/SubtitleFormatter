...
from utils import (
    read_file_safely, validate_file_path, get_file_info, 
    format_file_size, format_duration, print_separator,
...
class TextDiffer:
    ...
    def _show_file_info(self, old_file: str, new_file: str, 
    ...
    def _show_results(self, result: object, old_file: str, new_file: str, processing_time: float):
        ...
        if self.config.console_output:
            self.reporter.print_console_report(
                result, old_file, new_file, self.config.show_context
            )
    
    def save_reports(self, result: object, output_config: OutputConfig, 
        ...
        Args:
            ...
            new_file: 新文件路径
        """
        if not any([output_config.json_output, output_config.html_output, output_config.csv_output]):
            return
