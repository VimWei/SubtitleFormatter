...
from sequence_aligner import ComparisonResult, DiffType, WordDifference
...
class DiffReporter:
    ...
    def print_console_report(
        self, result: ComparisonResult, old_file: str, new_file: str, show_context: int = 3
    ):
        """
        打印控制台报告
    ...
    def _print_differences(self, differences: List[WordDifference], show_context: int):
        ...
        for i, diff in enumerate(differences, 1):
            ...
            if diff.type == DiffType.INSERT:
                ...
                print(f"  位置: 旧文件第{diff.old_line}行第{diff.old_column}列")
                print(
                    f"  内容: {self._colorize(f'- {diff.old_word}', 'red')} → {self._colorize(f'+ {diff.new_word}', 'green')}"
                )
    ...
    def save_html_report(
        self, result: ComparisonResult, filename: str, old_file: str, new_file: str
    ):
        """
        保存HTML格式报告
    ...
    def _generate_html_content(self, result: ComparisonResult, old_file: str, new_file: str) -> str:
        """生成HTML内容"""
        html = f"""
<!DOCTYPE html>
<html lang="zh-CN">
