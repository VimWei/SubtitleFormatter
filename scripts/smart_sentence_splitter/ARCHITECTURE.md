...
- **`is_simple_enumeration`**：智能识别简单并列，基于词汇数量和语法结构判断，避免误拆句子结构
  ...
  - 规则3：检查连续的非句子结构短词模式
  - 排除复合从句标记（in which、of which等）和从属连词（that、which等）
- **`_analyze_comma_context`**：分析逗号上下文，智能判断是否为简单并列
- **`_is_consecutive_similar_words`**：检测连续的同类型词汇模式，包含语法标记（'s', 't', 'd', 'll', 've', 're', 'm', 'n'）避免误判缩略词
...
- **复合从句标记识别**：正确识别"in which"、"of which"等复合从句标记，避免误判为简单并列
- **语法标记处理**：包含常见语法标记（'s', 't', 'd', 'll', 've', 're', 'm', 'n'），避免误判缩略词为简单并列
